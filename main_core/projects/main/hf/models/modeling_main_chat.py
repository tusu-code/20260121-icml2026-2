...
from transformers import (AutoModel, GenerationConfig, LlamaForCausalLM,
                          LlamaTokenizer, UpstreamLLM2ForCausalLM)
from transformers.modeling_outputs import CausalLMOutputWithPast
from transformers.modeling_utils import PreTrainedModel
from transformers.utils import ModelOutput, logging
from transformers import StoppingCriteriaList, StoppingCriteria

from .configuration_main_chat import MainChatConfig
...
class MainChatModel(PreTrainedModel):
    config_class = MainChatConfig
    main_input_name = 'pixel_values'
    base_model_prefix = 'language_model'
    _no_split_modules = ['InternVisionModel', 'LlamaDecoderLayer', 'InternLM2DecoderLayer',
                         'Phi3DecoderLayer', 'UpstreamLLM2DecoderLayer', 'SAM2']
    _supports_flash_attn_2 = True
    supports_gradient_checkpointing = True

    def __init__(self, config: MainChatConfig, vision_model=None, language_model=None, use_flash_attn=True):
        ...
        if language_model is not None:
            ...
            elif config.llm_config.architectures[0] == 'UpstreamLLM2ForCausalLM':
                self.language_model = UpstreamLLM2ForCausalLM(config.llm_config)
            elif config.llm_config.architectures[0] == 'UpstreamLLM3ForCausalLM':
                assert transformers.__version__ >= '4.56.0', 'Please upgrade transformers to >=4.56.0 for UpstreamLLM3 support.'
                from transformers import UpstreamLLM3ForCausalLM
                self.language_model = UpstreamLLM3ForCausalLM(config.llm_config)
            else:
    ...
    def wrap_llm_lora(self, r=128, lora_alpha=256, lora_dropout=0.05):
        ...
        elif self.llm_arch_name in ['UpstreamLLM2ForCausalLM', 'LlamaForCausalLM']:
    ...
    def generate(
        ...
        # ---------------------------------------------------------------------
        # Compatibility: some LLM backbones (e.g., InternLM2) may not properly
        # initialize `generation_config` under newer transformers, and
        # transformers' generation utils may still access `self.generation_config`
        # inside the LLM even when a `generation_config` is passed in.
        #
        # Ensure BOTH:
        # - `generation_config` arg is not None
        # - `self.language_model.generation_config` is not None
        # ---------------------------------------------------------------------
        if generation_config is None:
            generation_config = getattr(self, "gen_config", None)
        if generation_config is None:
            try:
                generation_config = GenerationConfig.from_model_config(self.language_model.config)
            except Exception:
                generation_config = GenerationConfig()
        # Fill transformers_version if missing
        try:
            if getattr(generation_config, "transformers_version", None) in (None, ""):
                generation_config.transformers_version = transformers.__version__
        except Exception:
            pass
        try:
            if getattr(self.language_model, "generation_config", None) is None:
                self.language_model.generation_config = generation_config
            else:
                if getattr(self.language_model.generation_config, "transformers_version", None) in (None, ""):
                    self.language_model.generation_config.transformers_version = transformers.__version__
        except Exception:
            pass

        # Ensure hidden states are actually produced by the LLM forward when requested.
        # Some backbones ignore `output_hidden_states` in generate kwargs unless also
        # enabled on the config.
        try:
        ...
        # Avoid passing None-valued keys that can confuse HF generation internals.
        # (Some backbones' prepare_inputs_for_generation are not robust to None/empty pkv.)
        if generate_kwargs.get("past_key_values", "___") is None:
            generate_kwargs.pop("past_key_values", None)
        if generate_kwargs.get("position_ids", "___") is None:
            generate_kwargs.pop("position_ids", None)
        if generate_kwargs.get("labels", "___") is None:
            generate_kwargs.pop("labels", None)

        # Do NOT force use_cache=True: some backbones (e.g., InternLM2) have
        # generation cache incompatibilities under newer transformers. Respect
        # generation_config if provided; default to False for robustness.
        _use_cache = False
        try:
            if generation_config is not None and getattr(generation_config, "use_cache", None) is not None:
                _use_cache = bool(getattr(generation_config, "use_cache"))
        except Exception:
            _use_cache = False
    ...
    def preparing_for_generation(self, tokenizer, max_new_tokens=2048, torch_dtype=torch.bfloat16):
        ...
        self.gen_config.transformers_version = '4.42.3'
        self.init_prediction_config = True
        self.torch_dtype = torch_dtype
        # self.to(torch_dtype)
        self.extra_image_processor = DirectResize(target_length=1024, )
        # for multi image process
        # Allow overriding image token budget at inference time.
        # This helps long expressions (e.g. RefCOCOg) where too many image tokens can
        # crowd out text tokens and effectively truncate the prompt tail.
        ...
        self.min_dynamic_patch = _env_int("MAIN_MIN_DYNAMIC_PATCH", 1)
        self.max_dynamic_patch = _env_int("MAIN_MAX_DYNAMIC_PATCH", 12)
        self.downsample_ratio = _env_float("MAIN_DOWNSAMPLE_RATIO", 0.5)
        self.image_size = _env_int("MAIN_IMAGE_SIZE", 448)
        self.use_thumbnail = _env_bool("MAIN_USE_THUMBNAIL", True)
        patch_size = 14
        self.patch_size = patch_size
    ...
    def predict_forward(
        ...
        #
        # IMPORTANT: `generate_output.hidden_states` layout differs across transformers versions/backbones.
        # There are two common patterns:
        #   (A) full-seq: last step returns (B, S, H) where S is full prompt+generated length.
        #   (B) last-token-per-step: each step returns (B, 1, H) (only the newly generated token).
        #
        # If we treat (B) as (A), `seg_hidden_states` becomes empty -> no masks -> eval falls back to all-zero masks.
        seg_hidden_states = _extract_seg_hidden_states_from_generate(
...
def _extract_seg_hidden_states_from_generate(generate_output, seg_id: int):
    """Extract hidden states for seg tokens from HF generate() output.

    Returns: (n_seg, hidden_dim) tensor on model device.
    """
    hidden_states = getattr(generate_output, "hidden_states", None)
    sequences = getattr(generate_output, "sequences", None)
    if hidden_states is None or sequences is None:
        raise ValueError("generate() must be called with output_hidden_states=True and return_dict_in_generate=True")
    # sequences: (B, S). We only support B=1 here (current code path).
    output_ids = sequences[0]
    # Drop the last token (often EOS) to match earlier behavior.
    if output_ids.numel() > 0:
        output_ids = output_ids[:-1]
    else:
        # Nothing generated / empty sequence.
        # Return an empty tensor with a valid hidden dim once we can infer it.
        # Best-effort: fall through to infer from hidden_states.
        pass

    # Try to read full sequence hidden states from the last step (pattern A).
    last_step = hidden_states[-1]
    last_layer = last_step[-1] if isinstance(last_step, (list, tuple)) else last_step
    if hasattr(last_layer, "dim") and last_layer.dim() == 3 and last_layer.shape[1] > 1:
        # (B, S, H)
        hs = last_layer[0]
        if hs.shape[0] >= output_ids.shape[0]:
            hs = hs[-output_ids.shape[0]:]
        else:
            # Unexpected; fall back to stepwise mode.
            hs = None
        if hs is not None:
            if output_ids.numel() == 0:
                return hs[0:0]
    ...
    # Stepwise last-token mode (pattern B): build per-generated-token hiddens.
    per_token = []
    for step in hidden_states:
        layer = step[-1] if isinstance(step, (list, tuple)) else step
        t = layer
        if not hasattr(t, "dim"):
            continue
        if t.dim() == 3:
            # (B, 1, H) or (B, S, H) - take last token
            per_token.append(t[0, -1, :])
        elif t.dim() == 2:
            # (S, H) - take last token
            per_token.append(t[-1, :])
    if len(per_token) == 0:
        return last_layer[0:0] if hasattr(last_layer, "__getitem__") else output_ids[0:0]
    token_hs = torch.stack(per_token, dim=0)  # (n_gen, H)
    n_gen = token_hs.shape[0]

    # Align generated tokens to the tail of output_ids.
    prompt_len = max(int(output_ids.shape[0] - n_gen), 0)
    gen_ids = output_ids[prompt_len:]
    if gen_ids.numel() == 0:
        return token_hs[0:0]
    # Keep shapes aligned (defensive against edge-cases where token counts mismatch).
    n = min(int(gen_ids.shape[0]), int(token_hs.shape[0]))
    gen_ids = gen_ids[-n:]
    token_hs = token_hs[-n:]
    seg_mask = gen_ids == seg_id
    if seg_mask.numel() == 0:
        return token_hs[0:0]
    return token_hs[seg_mask]
